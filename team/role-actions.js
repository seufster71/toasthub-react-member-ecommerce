...
export function init({parent,parentType,teamId}) {
	return function(dispatch) {
		...
		requestParams.service = "EC_ROLE_SVC";
		if (parentType != null) {
			requestParams.parentType = parentType;
		}
		if (parentType == "TEAM") {
			requestParams.prefTextKeys = new Array("EC_TEAM_ROLE_PAGE");
			requestParams.prefLabelKeys = new Array("EC_TEAM_ROLE_PAGE");
		} else if (parentType == "MEMBER") {
			requestParams.prefTextKeys = new Array("EC_MEMBER_ROLE_PAGE");
			requestParams.prefLabelKeys = new Array("EC_MEMBER_ROLE_PAGE");
			requestParams.teamId = teamId;
		}
		
		if (parent != null) {
			requestParams.parentId = parent.id;
			dispatch({type:"EC_ROLE_ADD_PARENT", parent, parentType});
		} else {
			dispatch({type:"EC_ROLE_CLEAR_PARENT"});
		}
		let params = {};
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "EC_ROLE_INIT", responseJson });
			} else {
...
export function list({state,listStart,listLimit,searchCriteria,orderCriteria,info,paginationSegment}) {
	return function(dispatch) {
		...
		dispatch({type:"EC_ROLE_PREF_CHANGE", prefChange});
		let params = {};
		params.requestParams = requestParams;
		params.URI = '/api/member/callService';
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "EC_ROLE_LIST", responseJson, paginationSegment });
				if (info != null) {
...
export function listLimit({state,listLimit}) {
	return function(dispatch) {
		 dispatch({ type:"EC_ROLE_LISTLIMIT",listLimit});
...
export function search({state,searchCriteria}) {
	return function(dispatch) {
		 dispatch({ type:"EC_ROLE_SEARCH",searchCriteria});
...
export function modifyItem({id, appPrefs}) {
	return function(dispatch) {
	    ...
	    requestParams.service = "EC_ROLE_SVC";
	    requestParams.prefFormKeys = new Array("EC_TEAM_ROLE_FORM");
	    if (id != null) {
	    	requestParams.itemId = id;
	    }
	    let params = {};
	    ...
	    return callService(params).then( (responseJson) => {
	    	if (responseJson != null && responseJson.protocalError == null){
	    		dispatch({ type: 'EC_ROLE_ITEM',responseJson, appPrefs});
	    	} else {
	    		actionUtils.checkConnectivity(responseJson,dispatch);
	    	}
	    }).catch(error => {
...
export function modifyMemberRole({role, appPrefs}) {
	return function(dispatch) {
	    ...
	    requestParams.service = "EC_ROLE_SVC";
	    requestParams.prefFormKeys = new Array("EC_MEMBER_ROLE_FORM");
	    ...
	    return callService(params).then( (responseJson) => {
	    	if (responseJson != null && responseJson.protocalError == null){
	    		dispatch({ type: 'EC_ROLE_MEMBER_ROLE',responseJson, appPrefs, role});
	    	} else {
	    		actionUtils.checkConnectivity(responseJson,dispatch);
	    	}
	    }).catch(error => {
...
export function modifyTeamRole({role, appPrefs}) {
	return function(dispatch) {
	    ...
	    return callService(params).then( (responseJson) => {
	    	if (responseJson != null && responseJson.protocalError == null){
	    		dispatch({ type: 'EC_ROLE_TEAM_ROLE',responseJson, appPrefs, role});
	    	} else {
	    		actionUtils.checkConnectivity(responseJson,dispatch);
	    	}
	    }).catch(error => {
...
export function inputChange(field,value) {
	 return function(dispatch) {
		 ...
		 params.field = field;
		 params.value = value;
		 dispatch({ type:"EC_ROLE_INPUT_CHANGE",params});
	 };
}

export function orderBy({state,orderCriteria}) {
	 return function(dispatch) {
		 dispatch({ type:"EC_ROLE_ORDERBY",orderCriteria});
...
export function clearItem() {
	return function(dispatch) {
		dispatch({ type:"EC_ROLE_CLEAR_ITEM"});
	};
}
...
export function clearField(field) {
	return function(dispatch) {
		let params = {};
		 params.field = field;
		dispatch({ type:"EC_ROLE_CLEAR_FIELD",params});
	};
}

export function setStatus({successes,errors}) {
	 return function(dispatch) {
		 dispatch({ type:"EC_ROLE_SET_STATUS",successes,errors});
	 };
}

export function openDeleteModal({item}) {
	 return function(dispatch) {
		 dispatch({type:"EC_ROLE_OPEN_DELETE_MODAL",item});
	 };
}

export function closeDeleteModal() {
	 return function(dispatch) {
		 dispatch({type:"EC_ROLE_CLOSE_DELETE_MODAL"});
	 };
}

export function cancel({state}) {
	return function(dispatch) {
		dispatch({type:"EC_ROLE_CANCEL"});
		dispatch(list({state}));
	 };
}