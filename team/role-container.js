...
import ECRoleView from '../../../memberView/ec/team/role-view';
import ECRoleModifyView from '../../../memberView/ec/team/role-modify-view';
import ECMemberRolesModifyView from '../../../memberView/ec/team/member-roles-modify-view';
import utils from '../../../core/common/utils';
import BaseContainer from '../../../core/container/base-container';


function ECRoleContainer({location,navigate}) {
	...
	const onSave = () => {
		BaseContainer.onSave({state:itemState,actions:actions,dispatch:dispatch,appPrefs:appPrefs,form:"EC_TEAM_ROLE_FORM"});
	}
	const closeModal = () => {
	...
	const onMemberRoleModify = (item) => {
		fuLogger.log({level:'TRACE',loc:'ECRoleContainer::onMemberRoleModify',msg:"test"+item.id});
	...
	const onTeamRoleModify = (item) => {
		fuLogger.log({level:'TRACE',loc:'ECRoleContainer::onTeamRoleModify',msg:"test"+item.id});
		if (item.memberRole != null) {
	...
	const onMemberRoleSave = () => {
		fuLogger.log({level:'TRACE',loc:'ECRoleContainer::onMemberRoleSave',msg:"test"});
		let errors = utils.validateFormFields(itemState.prefForms.EC_MEMBER_ROLE_FORM,itemState.inputFields, appPrefs.prefGlobal.LANGUAGES);
	...
	const onOption = (code,item) => {
		fuLogger.log({level:'TRACE',loc:'ECRoleContainer::onOption',msg:" code "+code});
	...
	fuLogger.log({level:'TRACE',loc:'ECRoleContainer::render',msg:"Hi there"});
...
export default ECRoleContainer;
