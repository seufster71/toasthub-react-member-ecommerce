	...
	switch(action.type) {
    	case 'EC_MEMBER_INIT': {
    		if (action.responseJson != null && action.responseJson.params != null) {
    			return Object.assign({}, state, {
    				...
    				orderCriteria: [{'orderColumn':'EC_MEMBER_TABLE_NAME','orderDir':'ASC'}],
    				searchCriteria: [{'searchValue':'','searchColumn':'EC_MEMBER_TABLE_NAME'}],
    				paginationSegment: 1,
    				selected: null,
    				view: "MAIN",
    	...
    	case 'EC_MEMBER_ITEM': {
			if (action.responseJson !=  null && action.responseJson.params != null) {
				...
				inputFields = reducerUtils.loadInputFields(action.responseJson.params.item,prefForms.EC_MEMBER_FORM,inputFields,action.appPrefs,"FORM1");
		...
		case 'EC_MEMBER_CLEAR_PARENT': {
			return Object.assign({}, state, {
				parent: null,
				parentType: null
			});
		}
		case 'EC_MEMBER_MEMBER_ROLE': {
			if (action.responseJson !=  null && action.responseJson.params != null) {
				// load inputFields
				let inputFields = {};
				let prefForms = reducerUtils.getPrefForms(action);
		...
		case 'EC_MEMBER_LISTLIMIT': {
			return reducerUtils.updateListLimit(state,action);
		}
		case 'EC_MEMBER_SEARCH': { 
			return reducerUtils.updateSearch(state,action);
		}
		case 'EC_MEMBER_SEARCH_CHANGE': { 
			return reducerUtils.updateSearchChange(state,action);
		}
		case 'EC_MEMBER_ORDERBY': { 
			return reducerUtils.updateOrderBy(state,action);
		}
		case 'EC_MEMBER_SET_ERRORS': {
			return Object.assign({}, state, {
				errors: action.errors
			});
		}
		case 'EC_MEMBER_CLOSE_DELETE_MODAL': {
			return Object.assign({}, state, {
				isDeleteModalOpen: false
			});
		}
		case 'EC_MEMBER_OPEN_DELETE_MODAL': {
			return Object.assign({}, state, {
				isDeleteModalOpen: true,
				selected: action.item
			});
		}
		case 'EC_MEMBER_CANCEL': {
			return Object.assign({}, state, {
				view: "MAIN",
				selected:null,
				inputFields:null
