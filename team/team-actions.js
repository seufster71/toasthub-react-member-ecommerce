...
export function init({parent,parentType}) {
	return function(dispatch) {
		...
		requestParams.service = "EC_TEAM_SVC";
		requestParams.prefTextKeys = new Array("EC_TEAM_PAGE");
		requestParams.prefLabelKeys = new Array("EC_TEAM_PAGE");
		if (parent != null) {
			requestParams.parentId = parent.id;
			requestParams.parentType = parentType;
			dispatch({type:"EC_TEAM_ADD_PARENT", parent, parentType});
		} else {
			dispatch({type:"EC_TEAM_CLEAR_PARENT"});
		}
		let params = {};
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "EC_TEAM_INIT", responseJson });
			} else {
...
export function list({state,listStart,listLimit,searchCriteria,orderCriteria,info,paginationSegment}) {
	return function(dispatch) {
		...
		dispatch({type:"EC_TEAM_PREF_CHANGE", prefChange});
		let params = {};
		params.requestParams = requestParams;
		params.URI = '/api/member/callService';
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "EC_TEAM_LIST", responseJson, paginationSegment });
				if (info != null) {
...
export function listLimit({state,listLimit}) {
	return function(dispatch) {
		 dispatch({ type:"EC_TEAM_LISTLIMIT",listLimit});
...
export function search({state,searchCriteria}) {
	return function(dispatch) {
		 dispatch({ type:"EC_TEAM_SEARCH",searchCriteria});
...
export function modifyItem({id, appPrefs}) {
	return function(dispatch) {
	    ...
	    requestParams.service = "EC_TEAM_SVC";
	    requestParams.prefFormKeys = new Array("EC_TEAM_FORM");
	    if (id != null) {
	    	requestParams.itemId = id;
	    }
	    let params = {};
	    ...
	    return callService(params).then( (responseJson) => {
	    	if (responseJson != null && responseJson.protocalError == null){
	    		dispatch({ type: 'EC_TEAM_ITEM',responseJson, appPrefs});
	    	} else {
	    		actionUtils.checkConnectivity(responseJson,dispatch);
	    	}
	    }).catch(error => {
...
export function modifyTeamLink({item, parentType, appPrefs}) {
	return function(dispatch) {
	    ...
	    requestParams.service = "EC_TEAM_SVC";
	    requestParams.prefFormKeys = new Array("EC_TEAM_PRODUCT_FORM");
	    ...
	    return callService(params).then( (responseJson) => {
	    	if (responseJson != null && responseJson.protocalError == null){
	    		dispatch({ type: 'EC_TEAM_LINK',responseJson, item, appPrefs, parentType});
	    	} else {
	    		actionUtils.checkConnectivity(responseJson,dispatch);
	    	}
	    }).catch(error => {
...
export function inputChange(field,value) {
	 return function(dispatch) {
		 ...
		 params.field = field;
		 params.value = value;
		 dispatch({ type:"EC_TEAM_INPUT_CHANGE",params});
	 };
}

export function orderBy({state,orderCriteria}) {
	 return function(dispatch) {
		 dispatch({ type:"EC_TEAM_ORDERBY",orderCriteria});
...
export function clearRole() {
	return function(dispatch) {
		dispatch({ type:"EC_TEAM_CLEAR_ROLE"});
	};
}

export function setErrors({errors}) {
	 return function(dispatch) {
		 dispatch({ type:"EC_TEAM_SET_ERRORS",errors});
	 };
}

export function openDeleteModal({item}) {
	 return function(dispatch) {
		 dispatch({type:"EC_TEAM_OPEN_DELETE_MODAL",item});
	 };
}

export function closeDeleteModal() {
	 return function(dispatch) {
		 dispatch({type:"EC_TEAM_CLOSE_DELETE_MODAL"});
	 };
}

export function cancel({state}) {
	return function(dispatch) {
		dispatch({type:"EC_TEAM_CANCEL"});
		dispatch(list({state}));
	 };
}